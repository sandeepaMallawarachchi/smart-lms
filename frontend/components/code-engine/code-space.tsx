...
const LANGUAGE_CONFIG: Record<string, { judge0_id: number }> = {
  javascript: { judge0_id: 93 }, // Node.js
  python: { judge0_id: 71 },     // Python
  java: { judge0_id: 62 },       // Java
  cpp: { judge0_id: 54 },        // C++
}
...
const formatAIFeedback = (text: string) => {
  ...
  let formatted = text
    ...
    .replace(/\s+(\d+\.\s)/g, '\n$1')
    .replace(/(✅|❌)\s/g, '$1&nbsp;')
    .replace(/```(?:\w+)?\s*([\s\S]*?)```/g, '<div class="my-3"><pre class="bg-gray-800 text-gray-50 p-3 rounded-xl text-xs font-mono overflow-x-auto shadow-sm tracking-normal"><code>$1</code></pre></div>')
    .replace(/`(?:javascript|python|java|cpp|html|css)?\s*([^`]+)`/g, '<code class="bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded text-xs font-mono border border-indigo-100">$1</code>')
    .replace(/###\s+([^\n]+)/g, '<h6 class="font-bold text-gray-900 mt-5 mb-2 text-sm uppercase tracking-wide border-b border-gray-100 pb-1.5">$1</h6>')
...
const analyzeCodeWithAI = async (code: string, language: string): Promise<AnalysisResult> => {
  try {
    ...
    let codeToSend = code;
    if (codeLines.length > 0 && (codeLines[0].trim().startsWith('//') || codeLines[0].trim().startsWith('#') || codeLines[0].trim().startsWith('/*'))) {
      codeToSend = codeLines.slice(1).join('\n').trim();
    }
    ...
    let metrics = { score: 70, time_complexity: 'O(unknown)', space_complexity: 'O(unknown)' };
    if (metricsRes && metricsRes.ok) {
      metrics = await metricsRes.json();
    }

    let auditText = "No feedback received.";
    if (auditRes && auditRes.ok) {
      const auditData = await auditRes.json();
      auditText = auditData.analysis || "No feedback received.";
    }

    return {
      score: metrics.score,
      complexity: {
        time: metrics.time_complexity,
        space: metrics.space_complexity
      },
      feedback: [{
        type: metrics.score >= 80 ? 'success' : metrics.score >= 50 ? 'warning' : 'info',
        message: 'AI Code Review',
        description: auditText
      }],
      suggestions: [
        'Review the AI feedback above for detailed suggestions.'
      ]
    };
  } catch (err) {
    ...
    return {
      ...
      feedback: [{ type: 'info', message: 'Analysis Failed', description: 'Could not connect to AI backend. Make sure the local python server is running on port 8000.' }],
      suggestions: []
    }
  }
};
...
const CodeSpace = ({ defaultCode, assignment }: CodeSpaceProps) => {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("RapidAPI key is missing in environment variables.");
      }
      ...
      const promises = assignment.testCases.map(async (testCase) => {
        ...
        if (data.message && data.message.includes("not subscribed")) {
          throw new Error("RapidAPI key is not subscribed to Judge0 CE.");
        }

        const rawOutput = data.stdout ? data.stdout : '';
        const cleanActual = rawOutput.trim();
        const cleanExpected = testCase.expectedOutput.trim();
        const isError = data.status?.id !== 3; // 3 is "Accepted"

        let stderrMsg = data.stderr || data.compile_output || '';
        if (isError && !stderrMsg) {
          stderrMsg = data.status?.description || 'Execution Error';
        }
        ...
        return {
          ...
          actual: rawOutput,
          passed: cleanActual === cleanExpected && !isError,
          isError: isError,
          stderr: stderrMsg,
      ...
      console.error(err);
      setStatusMessage(`Error: ${err.message || 'Could not reach execution engine.'}`);
    } finally {
      setIsRunning(false);
    }
  }
                      ...
                      <div className="space-y-3">
                        {analysisResult.feedback.map((item, i) => (
                          <motion.div
                            ...
                            className={`p-4 rounded-xl border flex items-start gap-3.5 transition-all hover:shadow-md ${item.type === 'success' ? 'bg-emerald-50/50 border-emerald-100' :
                              item.type === 'warning' ? 'bg-amber-50/50 border-amber-100' :
                                'bg-blue-50/50 border-blue-100'
                              }`}
                          >
                            <div className={`mt-0.5 p-1.5 rounded-full shrink-0 ${item.type === 'success' ? 'bg-emerald-100 text-emerald-600' :
                            ...
                            <div className="flex-1 min-w-0">
                              <h5 className={`text-sm font-bold mb-1 ${item.type === 'success' ? 'text-emerald-900' :
                                item.type === 'warning' ? 'text-amber-900' :
                                  'text-blue-900'
                                }`}>{item.message}</h5>
                              <div
                                className="text-xs text-gray-700 leading-relaxed overflow-hidden max-w-full"
                                dangerouslySetInnerHTML={formatAIFeedback(item.description)}
                              />
                            </div>
                          </motion.div>
                        ))}
