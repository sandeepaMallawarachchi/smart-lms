    ...
    return (
        <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
                ...
                <span className={`text-sm font-semibold tabular-nums ${scoreLabelColour(score)}`}>
                    {score.toFixed(1)} / 10
                </span>
            </div>
            ...
            <ul className="space-y-1.5">
                {items.map((item, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                        <span className={`mt-1.5 h-2 w-2 flex-shrink-0 rounded-full ${dotColour}`} />
                        {item}
                    </li>
                ))}
            </ul>
...
export function LiveFeedbackPanel({ feedback, loading }: LiveFeedbackPanelProps) {
    ...
    if (!feedback && !loading) {
        return (
            <div className="flex flex-col items-center justify-center h-full min-h-[240px] rounded-lg border border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                ...
                <p className="text-base font-medium text-gray-700">AI Feedback</p>
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-full min-h-[240px] rounded-lg border border-gray-100 bg-gray-50 p-8 text-center">
                <div className="mb-4 h-10 w-10 animate-spin rounded-full border-2 border-purple-200 border-t-purple-500" />
    ...
    return (
        <div className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm flex flex-col gap-5">
            ...
            <div className="flex items-center gap-2.5">
                ...
                <span className="text-base font-semibold text-gray-800">AI Feedback</span>
                <span className="ml-auto text-sm text-gray-400">Live</span>
            </div>
